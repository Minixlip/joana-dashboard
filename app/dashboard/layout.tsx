...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const SidebarContent = () => (
    <>
      <div className='p-6 border-b border-gray-700'>
        <Link
          href='/dashboard'
          className='text-2xl font-serif font-semibold text-[#bfa76f]'
        >
          Joana&#39;s Dashboard
        </Link>
      </div>
  ...
  return (
    <div className='min-h-screen flex bg-primary-background text-primary-text'>
      ...
      {/* Main Content */}
      <main className='flex-1 p-6 sm:p-8 lg:p-10 overflow-y-auto bg-[#0a0a0a] border-l-[#bfa76f] border-l '>
        <div
          className={`transition-opacity duration-300 ${
            isSidebarOpenMobile && 'opacity-30 md:opacity-100'
          }`}
        >
          {children}
        </div>
